...
def points_window_overlap(points: np.ndarray, window: np.ndarray) -> float:
    if len(points) == 0:
        return 0.0

    p_bbox = points_bbox(points).reshape([1, 4])
    xmin, ymin, xmax, ymax = window
    w, h = xmax - xmin, ymax - ymin
    area = w * h
    window = np.array([ymin, xmin, ymax, xmax]).reshape([1, 4])
    score = intersection_area(p_bbox, window)
    return float(score[0, 0]) / area
...
def argmax_iou_matching(
    ...
    if boxes_a.shape[0] == 0:
        return (
            ...
            -np.ones([boxes_b.shape[0]], dtype=np.int64),
        )
    if boxes_b.shape[0] == 0:
        return (
            -np.ones([boxes_a.shape[0]], dtype=np.int64),
            np.zeros([0], dtype=np.int64),
        )
