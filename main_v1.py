...
<CameraCanvas>:
    ...
    #     center: self.parent.center[0], root.top - self.size[1]
    # 
    # ScoreGauge:
    #     id: score_gauge
    #     size: (100, 100)
    #     pos: 10, root.top - self.size[1] - 10
"""
...
class CameraCanvas(Widget):
    # score_gauge: ScoreGauge = ObjectProperty(None)
    camera_widget: CameraWidget = ObjectProperty(None)
    # overlap_gauge: OverlapGauge = ObjectProperty(None)
    matched_points = ListProperty([])
    # Other properties
    group: Optional[InstructionGroup] = None

    def update_overlay(self, score: float, points: np.ndarray):

        # self.score_gauge.score_text = f"{int(score)}"
        # anim = Animation(score=2 * int(score), duration=0.5)
        # anim.start(self.score_gauge)
        #
        # self.overlap_gauge.update_overlap(points)
...
class MainWindow(Screen):
    ...
    def analyse_photo(self, key: str, image: np.ndarray, resize: bool = True):
        ...
        keypoints, des = self.fe.detectAndCompute(gray, None)
    ...
    def stitch(self, *args):
        ...
        if sum(matchesMask) > 40:
            img2, _ = transform.cv_blend_images(img2, img1, np.linalg.inv(M))
            self.analyse_photo("pano_proposal", img2, resize=False)
            return True
        else:
            Snackbar(
                text=f"Not enough matches ({sum(matchesMask)}) !",
                bg_color=(1, 0, 0, 0.5),
                duration=2,
            ).open()
            return False
    ...
    def infinite_loop(self):
        ...
        while True:
            ...
            score, points = self.match_score()
            if not status or score is None:
                self.camera_canvas.update_overlay(0.0, [])
                time.sleep(0.5)
            else:
                print(f"Iteration {iteration} ")
                self.camera_canvas.update_overlay(score, points)
                time.sleep(0.5)
    ...
    def match_score(self):
        ...
        return matching_score, normalized_points
